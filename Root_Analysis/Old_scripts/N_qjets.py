...
for i, inputFile in enumerate(inputFiles):
    ...
    hist_jet_number = ROOT.TH1F("hist_jet_number", f"Number of Light quark Jets", 60, 1, 15)
...
############################################################
    ...
    for event in range(numberOfEntries):
        ...
        # Contar o número de jatos no evento atual
        num_jets_flavorq = 0
        for jetIndex in range(branchJet.GetEntries()):
            jetObject = branchJet.At(jetIndex)
            if abs(jetObject.Flavor) in [1,2,3,4]   and  jetObject.PT >= 30 and abs(jetObject.Eta) <= 3:

                num_jets_flavorq += 1  
        # Preencher o histograma com o número de jatos no evento atual
        hist_jet_number.Fill(num_jets_flavorq)

        print(f"Número de jatos no evento {event}:", num_jets_flavorq)
  ...
  #  hist_jet_number.SetLineWidth(2)
    ...
    image_path = os.path.join(output_dir, f"q-jet_number_perevent_{os.path.basename(inputFile).replace('.root', '.png')}")
