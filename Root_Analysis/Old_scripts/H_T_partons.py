...
output_dir = os.path.join(script_dir, "HT_allpartons")
...
colors = [ROOT.kCyan,  ROOT.kGray, ROOT.kRed, ROOT.kBlack, ROOT.kMagenta, ROOT.kBlue, ROOT.kOrange+4, ROOT.kViolet+5, ROOT.kGreen+1]
...
for i, inputFile in enumerate(inputFiles):
    ...
    histogram_HT = ROOT.TH1F("hist_ht", "H_{T}(TOTAL) ; H_{T}(TOTAL) (GeV); Number of events (L = 200 fb^{-1})" , 80, 0, 5000)
    ...
    for entry in range(numberOfEntries):
        ...
        # Obter o galho "Jet"
        branchJet = treeReader.UseBranch("Particle")
        
        # Imprimir PT e η dos jatos antes do corte
        for jet in branchJet:
            print(f"Jet PT: {jet.PT}, Jet Eta: {jet.Eta}")

        jets = [jet for jet in branchJet if jet.Status == 23]
        
        # Imprimir PT e η dos jatos após o corte
        for jet in jets:
            print(f"Selected Jet PT: {jet.PT}, Selected Jet Eta: {jet.Eta}")
