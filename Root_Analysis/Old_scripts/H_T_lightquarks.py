...
output_dir = os.path.join(script_dir, "HT_light_quarks")
...
for i, inputFile in enumerate(inputFiles):
    ...
    histogram_HT = ROOT.TH1F("hist_ht", "H_{T}(light-quarks) ; H_{T}(light-quarks) (GeV); Number of events (L = 200 fb^{-1})" , 80, 0, 2000)
    ...
    for entry in range(numberOfEntries):
        ...
        # Obter o galho "Jet"
        branchJet = treeReader.UseBranch("Particle")# na verdade o nome é jato, mas o objeto é parton
       
        jets = [jet for jet in branchJet if abs(jet.PID) in [1,2,3,4] and jet.Status == 23]
...
output_file_path_HT = os.path.join(output_dir, "HTq.png")
