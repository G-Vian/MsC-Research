...
for i, inputFile in enumerate(inputFiles):
    ...
    numberOfEntries = treeReader.GetEntries()
      ...
      # Criando histogramas para PT e ETA dos jato b 
    histogram_HT = ROOT.TH1F("hist_M", "M of pairs of bottom jets; M (GeV/c^{²}); Number of events", 50, 0, 500)
    for event in range(numberOfEntries):
        # Obter o evento atual
        treeReader.ReadEntry(event)
        ...
        if jets:
            ...
            for jet in jets:
                ...
                for combination in combinacoes:
                    jet1, jet2 = combination
                    if jet1 != jet2 and abs(jet.Flavor) == 5: 
...
# Salvando os histogramas em arquivos de imagem
output_file_path_HT = os.path.join(output_dir, "M_b.png")
canvas_HT.SaveAs(output_file_path_HT)
# Abra os histogramas quando prontos
subprocess.Popen(["xdg-open", output_file_path_HT])
...
for i, inputFile in enumerate(inputFiles):
    ...
    numberOfEntries = treeReader.GetEntries()
      ...
      # Criando histogramas para PT e ETA dos jato b 
    histogram_HT = ROOT.TH1F("histogram", "M bottom jets pairs closer to m_{H}; M (GeV/c^{²}); Number of events", 50, 0, 500)
    for event in range(numberOfEntries):
        # Obter o evento atual
        treeReader.ReadEntry(event)
        ...
        # Verificar se há massas invariantes suficientes para preencher o histograma
            if len(massas_invariantes) >= 2:
                ...
                massas_invariantes.sort(key=lambda x: abs(x - mh))
                ...
                histogram2.Fill(massa_selecionada_1, peso_atual)
                histogram2.Fill(massa_selecionada_2, peso_atual)
...
for i, inputFile in enumerate(inputFiles):
    ...
    numberOfEntries = treeReader.GetEntries()
      ...
      # Criando histogramas para PT e ETA dos jato b 
    histogram_HT = ROOT.TH1F("histogram", "M of pairs of light quark jets; M (GeV/c^{²}); Number of events", 50, 0, 500)
...
for i, inputFile in enumerate(inputFiles):
    ...
    numberOfEntries = treeReader.GetEntries()
      ...
      # Criando histogramas para PT e ETA dos jato b 
    histogram_HT = ROOT.TH1F("histogram", "M light quark jets pairs closer to m_{W}; M (GeV/c^{²}); Number of events", 50, 0, 500)
    ...
    for event in range(numberOfEntries):
        ...
        # Verificar se há massas invariantes suficientes para preencher o histograma
            if len(massas_invariantes) >= 2:
                ...
                massa_selecionada_2 = massas_invariantes[1]
                # Preencher o histograma com as duas massas selecionadas
                histogram2.Fill(massa_selecionada_1, 0.0041)
                histogram2.Fill(massa_selecionada_2, 0.0041)
                # Limpar a lista de massas invariantes para o próximo evento
                massas_invariantes.clear()
