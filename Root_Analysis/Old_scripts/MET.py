...
output_dir = os.path.join(script_dir, "MET")
...
for i, inputFile in enumerate(inputFiles):
    ...
    histogram_HT = ROOT.TH1F("hist_ht", "Missing transverse energy ; MET (GeV); Number of events (L = 200 fb^{-1})", 80, -20, 500)

    for event in range(numberOfEntries):  #Para um evento dentro do numero de eventos -- se nao ele fica em loop infinito
        # Obter o evento atual
        treeReader.ReadEntry(event)               #Lê o evento atual
        # Obter o ramo MeT
        branchMissingET = treeReader.UseBranch("MissingET")     #Acessa o galho do Missing ET
        branchJet = treeReader.UseBranch("Jet")

        # Lista para armazenar os MET deste evento
        MET_values = []        
       
        jets = [jet for jet in branchJet if  jet.PT >= 30 and abs(jet.Eta) <= 2.5]

        if len(jets) >= 6:
            print("A lista `jets` contém agora 6 ou mais jatos.")
            # Iterar sobre as MET do evento
            for metIndex in range(branchMissingET.GetEntries()):         # Cria um indice pra varrer sobre as variáveis do galho
                    # Obter o MET atual
                    metObject = branchMissingET.At(metIndex)        #vinula um objeto met a cada indice varrido do galho
                    MET_values.append(metObject.MET)                #Adiciona a lista MET values os objetos que são .MET , ie a missing energy. Podemos criar qualquer objeto e qualquer inddice para varrer os galhos
                                                                    #O indice varre todas as folhas do galho e quando acha arquivo .MET Ele adiciona a lista.
            for met in MET_values:                                  # Dizemos que a variável met está na lista MET_values e para cada valor dessa variável enchemos o histograma
                histogram_HT.Fill(met, peso_atual)


        else:
            print("Não foi possível encontrar jatos adicionais suficientes para atingir 6 elementos na lista `jets`.")
...
canvas_HT.SetRightMargin(0.15)
canvas_HT.SetBottomMargin(0.1)
...
for i, histogram_HT in enumerate(histograms_1):
    ...
    if i != len(histograms_1) - 1:
        histogram_HT.SetFillColor(colors[i])
