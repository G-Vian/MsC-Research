...
output_dir = os.path.join(script_dir, "HT_bquark")
...
for i, inputFile in enumerate(inputFiles):
    ...
    histogram_HT = ROOT.TH1F("hist_ht", "H_{T}(b-quarks) ; H_{T}(b-quarks) (GeV); Number of events (L = 200 fb^{-1})" , 80, 0, 2000)
    ...
    for entry in range(numberOfEntries):
        ...
        jets = [jet for jet in branchJet if abs(jet.PID) == 5 and jet.Status == 23]
...
output_file_path_HT = os.path.join(output_dir, "HTb.png")
