...
for i, inputFile in enumerate(inputFiles):
    ...
    hist_jet_number = ROOT.TH1F("hist_jet_number", f"Number of Jets", 60, 2, 25)
...
############################################################
    ...
    for event in range(numberOfEntries):
        ...
        hist_jet_number.Fill(num_jets)

        print(f"Número de jatos no evento {event}:", num_jets)
  ...
  #  hist_jet_number.SetLineWidth(2)
    ...
    image_path = os.path.join(output_dir, f"jet_number_perevent_{os.path.basename(inputFile).replace('.root', '.png')}")
