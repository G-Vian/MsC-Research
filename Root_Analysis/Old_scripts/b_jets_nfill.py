...
output_dir = os.path.join(script_dir, "b_jets_nfill")
...
for i, inputFile in enumerate(inputFiles):
    ...
    numberOfEntries = treeReader.GetEntries()
      ...
      # Criando histogramas para PT e ETA dos jato b 
    histogram_PT = ROOT.TH1F("hist_pt", "P_{T} jb[4]; P_{T} (GeV/c); Number of events (L = 200 fb^{-1})", 70,0, 600)
...
for i, inputFile in enumerate(inputFiles):
    ...
    numberOfEntries = treeReader.GetEntries()
      ...
      # Criando histogramas para PT e ETA dos jato b 
    histogram_PT = ROOT.TH1F("hist_pt", "P_{T} jb[5]; P_{T} (GeV/c); Number of events (L = 200 fb^{-1})", 70,0, 600)
...
for i, inputFile in enumerate(inputFiles):
    ...
    numberOfEntries = treeReader.GetEntries()
      ...
      # Criando histogramas para PT e ETA dos jato b 
    histogram_PT = ROOT.TH1F("hist_pt", "P_{T} jb[7]; P_{T} (GeV/c); Number of events (L = 200 fb^{-1})", 70, 0, 200)
    histogram_ETA = ROOT.TH1F("hist_pt", "#eta jb[7]; #eta ; Number of events (L = 200 fb^{-1})", 70, -10, 10)
    ...
    for entry in range(numberOfEntries):
        ...
        for jet in jets:
            # Verificando se a partícula é um jato b
            if  abs(jet.Flavor) == 5 and jet.PT >= 30>= 30  and abs(jet.Eta) <= 2.5 :
                pt_b.append(jet.PT)
                eta_b.append(jet.Eta)
...
# Salvando os histogramas em arquivos de imagem
output_file_path_PT = os.path.join(output_dir, "jb[7]_pt.png")
canvas_PT.SaveAs(output_file_path_PT)

output_file_path_ETA = os.path.join(output_dir, "jb[7]_eta.png")
canvas_ETA.SaveAs(output_file_path_ETA)
...
for i, inputFile in enumerate(inputFiles):
    ...
    numberOfEntries = treeReader.GetEntries()
      ...
      # Criando histogramas para PT e ETA dos jato b 
    histogram_PT = ROOT.TH1F("hist_pt", "P_{T} jb[8]; P_{T} (GeV/c); Number of events (L = 200 fb^{-1})", 70, 0, 200)
    histogram_ETA = ROOT.TH1F("hist_pt", "#eta jb[8]; #eta ; Number of events (L = 200 fb^{-1})", 70, -10, 10)
...
# Salvando os histogramas em arquivos de imagem
output_file_path_PT = os.path.join(output_dir, "jb[8]_pt.png")
canvas_PT.SaveAs(output_file_path_PT)

output_file_path_ETA = os.path.join(output_dir, "jb[8]_eta.png")
canvas_ETA.SaveAs(output_file_path_ETA)
