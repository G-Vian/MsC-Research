...
delta_h = 5  # Z and W have similar decay widths
mh = 80.3
...
for i, inputFile in enumerate(inputFiles):
    ...
    hist1 = ROOT.TH1F("hist1", "Invariant mass of W[1]", 50, 70, 90)
    hist2 = ROOT.TH1F("hist2", "Invariant mass of W[2]", 50, 70, 90)
    ...
    for event in range(numberOfEntries):
        ...
        jets = [jet for jet in branchJet if abs(jet.Flavor) in [0,1,2,3,4] and jet.PT >= 30]
        
        # Certificar-se de que existem pelo menos quatro jatos
        if len(jets) < 4:
            # Selecionar jatos que têm PT menor que 30
            additional_jets = [jet for jet in branchJet if abs(jet.Flavor) in [0,1,2,3,4] and jet.PT < 30]
        ...
        # Caso ainda não haja quatro jatos, continuar para o próximo evento
        if len(jets) < 4:
            continue

        # Calcular combinações de jatos e suas massas invariantes
        ...
        for (massa1, jet1a, jet1b), (massa2, jet2a, jet2b) in pares_de_pares:
            ...
            if len({jet1a, jet1b, jet2a, jet2b}) == 4:
                # Calcular X
                X = math.sqrt(((massa1 - mh)**2 / delta_h**2) + ((massa2 - mh)**2 / delta_h**2))
        ...
        if valores_X:
            ...
            if abs(massa1 - mh) <= delta_h and abs(massa2 - mh) <= delta_h:
...
output_file_path_1 = os.path.join(output_dir, "mw1_1.png")
...
# Salvando o histograma em um arquivo de imagem
output_file_path_2 = os.path.join(output_dir, "mw2_1.png")
