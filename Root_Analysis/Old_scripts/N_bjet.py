    ...
    for event in range(numberOfEntries):
        ...
        for jetIndex in range(branchJet.GetEntries()):
            ...
            if abs(jetObject.Flavor) == 5:
                print(f"Jato {jetIndex}: Flavor={jetObject.Flavor}, PT={jetObject.PT}, Eta={jetObject.Eta}")
            
            if abs(jetObject.Flavor) == 5 and jetObject.PT >= 30 and abs(jetObject.Eta) <= 2.5:
                num_jets_flavor5 += 1
