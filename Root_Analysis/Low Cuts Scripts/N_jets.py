...
with open(output_file_path, "a") as output_file:
    ...
    for i, inputFile in enumerate(inputFiles):
        ...
        hist_jet_number = ROOT.TH1F("hist_jet_number", f"Number of Jets ({os.path.basename(inputFiles[i]).replace('100.root', '')})", 40, 1, 15)
        ...
        for entry in range(numberOfEntries):
            ...
            #SEGUNDA CONDICIONAL, DENTRO DA PRIMEIRA, QUE OBSERVA O NUMERO DE OBJETOS
                if len(q_jets) >= 0 and len(b_jets) >= 0 and len(muons) == 0 and len(electrons) == 0 : 
                    ...
                    print(f"Número de jatos no evento {entry}:", num_jets)
