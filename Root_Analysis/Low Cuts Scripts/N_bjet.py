...
#f"  {os.path.basename(inputFiles[i]).replace('100.root', '')}", "f100"
    for i, inputFile in enumerate(inputFiles):
        print(f"Reading file {inputFile}")
        hist_jet_number = ROOT.TH1F("hist_jet_number", f"Number of b- Jets ({os.path.basename(inputFiles[i]).replace('100.root', '')})", 40, 1, 10)
    ...
    #  hist_jet_number.SetLineWidth(2)
        ...
        legend_PT = ROOT.TLegend(0.75, 0.1, 0.9, 0.3)  
