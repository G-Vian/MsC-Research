    ...
    for i, inputFile in enumerate(inputFiles):
       ...
       # OBTENDO OS GALHOS
        ...
        for entry in range(numberOfEntries):
            ...
            if   nJets >= 6 and HT_value > 0 : 
                events_passed_MET_HT_nJets += 1 #pra eu contar quantos eventos passaram
                selected_jets = [jet for jet in branchJet if jet.PT >= 20 and abs(jet.Eta) <= 2.5 ]        
...
canvas_ETA.SetRightMargin(0.22)
...
#################################################################
    ...
    legend_PT = ROOT.TLegend(0.825, y1 - i * (y1 - y2) / n_entries, 0.90, y1 - (i + 1) * (y1 - y2) / n_entries)
...
text.DrawLatex(0.75, 0.01, f"R: {R1:.3f}")
...
text.SetNDC()
text.SetTextSize(0.03)
text.DrawLatex(0.75, 0.01, f"R: {R2:.3f}")
