...
output_dir = os.path.join(script_dir, "HT")
...
#lista_de_pesos =  [P_1, P_3, P_5, P_6, P_2, P_4, P_7, P_8, P_12, P_10, P_13, P_9, P_11] #com corte 
#colors =  [  ROOT.kMagenta+3, ROOT.kOrange+4, ROOT.kGreen+3, ROOT.kAzure+3,  ROOT.kMagenta,  ROOT.kBlue, ROOT.kOrange+7, ROOT.kTeal-3,  ROOT.kBlack, ROOT.kYellow,  ROOT.kCyan, ROOT.kViolet, ROOT.kRed ]  #com corte
...
with open(output_file_path, "a") as output_file:
    ...
    for i, inputFile in enumerate(inputFiles):
        ...
        histogram_HT = ROOT.TH1F("hist_ht", "H_{T}(TOTAL) ; H_{T}(TOTAL) (GeV); Number of events (L = 200 fb^{-1})" , 20, 0, 4000)
        ...
        for entry in range(numberOfEntries):
            ...
            #SEGUNDA CONDICIONAL, DENTRO DA PRIMEIRA, QUE OBSERVA O NUMERO DE OBJETOS
                if len(q_jets) >= 4 and len(b_jets) >= 6  and len(muons) == 0 and len(electrons) == 0 : 
                    events_passed_final_selection += 1  # Incrementa o contador para eventos que passam essa seleção final

                    histogram_HT.Fill(HT_value, peso_atual)
        ...
        print(f"Eventos que passaram MET >= 40, HT > 500 e nJets >=  0: {events_passed_MET_HT_nJets}")
        ...
        output_file.write(f"Eventos que passaram MET >= 40, HT > 500 e nJets >=  0: {events_passed_MET_HT_nJets}\n")
        ...
        total_events = peso_atual*histogram_HT.GetEntries()
...
#################################################################
    ...
    legend_PT.AddEntry(histogram_HT, f"  {os.path.basename(inputFiles[i]).replace('100.root', '')}", "f")
...
output_file_path_HT = os.path.join(output_dir, "HT.png")
