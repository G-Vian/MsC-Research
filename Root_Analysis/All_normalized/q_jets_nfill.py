...
output_dir = os.path.join(script_dir, "q_jets_nfill")
...
with open(output_file_path2, "a") as output_file2:
    ...
    for i, inputFile in enumerate(inputFiles):
        ...
        # Criando histogramas para PT e ETA dos jato b 
        histogram_PT = ROOT.TH1F("hist_pt", "P_{T} jq[2]; P_{T} (GeV/c); Number of events (L = 200 fb^{-1})", 70,0, 800)
       ...
       # OBTENDO OS GALHOS
        ...
        for entry in range(numberOfEntries):
            ...
            nJets = branchJet.GetEntries()
            ht_object = branchHT.At(0)  # Assume-se que há apenas um objeto de HT por evento
            HT_value = ht_object.HT        #jet_PT = branchJet.PT #N3ão precisa pra esses objetos
            #jet_ETA = branchJet.ETA
            #elec_PT = branchelec.PT
            #mu_PT = branchmu.PT
            #elec_ETA = branchelec.ETA
...
with open(output_file_path3, "a") as output_file3:
    ...
    for i, inputFile in enumerate(inputFiles):
        ...
        # Criando histogramas para PT e ETA dos jato b 
        histogram_PT = ROOT.TH1F("hist_pt", "P_{T} jq[3]; P_{T} (GeV/c); Number of events (L = 200 fb^{-1})", 70,0, 800)
...
with open(output_file_path3, "a") as output_file3:
    ...
    for i, inputFile in enumerate(inputFiles):
        ...
        # Criando histogramas para PT e ETA dos jato b 
        histogram_PT = ROOT.TH1F("hist_pt", "P_{T} jq[4]; P_{T} (GeV/c); Number of events (L = 200 fb^{-1})", 70,0, 500)
