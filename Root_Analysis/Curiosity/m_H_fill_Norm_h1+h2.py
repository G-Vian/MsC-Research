...
with open(output_file_path, "a") as output_file:
    ...
    # Histogramas globais para todos os eventos
        hist1 = ROOT.TH1F("hist1", "Invariant mass of all h NO X^2 SELECTION;  M_{j_b j_b} (GeV); Number of events (Normalized to one)", 70, 0, 700)
        ...
        for entry in range(numberOfEntries):
            ...
            #SEGUNDA CONDICIONAL, DENTRO DA PRIMEIRA, QUE OBSERVA O NUMERO DE OBJETOS
                if len(q_jets) >= 2 and len(b_jets) >= 4 and len(muons) == 0 and len(electrons) == 0 : 
                    ...
                    for mass, jet1, jet2 in massas_invariantes: #pq mass e não invariant_mass? é como se invariant_mass(kjet1, jet2) fosse um operador que retorna mass como autovalor
                        hist1.Fill(mass, peso_atual)
...
output_file_path_1 = os.path.join(output_dir, "mh1+h2_fill_norm.png")
