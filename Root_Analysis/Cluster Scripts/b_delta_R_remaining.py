...
output_file_path1 = os.path.join(output_dir, "processing_log14.txt")
...
output_file_path2 = os.path.join(output_dir, "processing_log15.txt")
...
with open(output_file_path1, "a") as output_file1:
    ...
    for i, inputFile in enumerate(inputFiles):
        ...
        histogram_PT = ROOT.TH1F(f"hist_pt_{i}", "#Delta R jb[4] and jb[6]; #Delta R ; Number of events (L = 200 fb^{-1})", 40, 0, 9)
...
################################################################## #NOVO PADRÃO
    ...
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(0, f"Entr.: {N_entries[i]:.1f}, Mod. V: {modal_values[i]:.1f}", "")
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(0, "", "")
    legend_PT.Draw()
    legend_PT.Draw()
...
with open(output_file_path2, "a") as output_file2:
    ...
    for i, inputFile in enumerate(inputFiles):
        ...
        # Criando histogramas para PT e ETA dos jato b
        histogram_PT = ROOT.TH1F(f"hist_pt_{i}", "#Delta R jb[5] and jb[6]; #Delta R ; Number of events (L = 200 fb^{-1})", 40, 0, 9)
        # histogram_ETA = ROOT.TH1F(f"hist_eta_{i}", "#eta jb[1]; #eta ; Number of events (L = 200 fb^{-1})", 40, -10, 10)
                    # OBTENDO OS GALHOS
        branchJet = treeReader.UseBranch("Jet")
        branchMissingET = treeReader.UseBranch("MissingET")
