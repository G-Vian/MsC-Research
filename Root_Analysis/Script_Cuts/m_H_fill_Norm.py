...
lista_de_pesos = [P_6, P_7, P_3,  P_4, P_8, P_10, P_12, P_13, P_11] #sem tt , ttbb & ttWW

#colors =  [   ROOT.kOrange+4, ROOT.kBlue,  ROOT.kGreen+3, ROOT.kAzure+3,  ROOT.kOrange+7, ROOT.kTeal-3, ROOT.kYellow, ROOT.kBlack, ROOT.kCyan,  ROOT.kRed ] 
colors = [ ROOT.kAzure+3, ROOT.kOrange+7, ROOT.kOrange+4,  ROOT.kBlue, ROOT.kTeal-3,ROOT.kYellow, ROOT.kBlack, ROOT.kCyan, ROOT.kRed ]

#colors =  [ 1 ROOT.kMagenta+3, 2 ROOT.kMagenta, 3 ROOT.kOrange+4, 4 ROOT.kBlue, 5 ROOT.kGreen+3, 6 ROOT.kAzure+3, 7 ROOT.kOrange+7, 8 ROOT.kTeal-3, 9 ROOT.kViolet, 10 ROOT.kYellow, 12 ROOT.kBlack, 13 ROOT.kCyan, 11 ROOT.kRed ]  
#ROOT.kBlue,ROOT.kOrange+4, ROOT.kViolet+5, ROOT.kAzure-7]
...
with open(output_file_path, "a") as output_file:
    ...
    # Histogramas globais para todos os eventos
        hist1 = ROOT.TH1F("hist1", "Invariant mass of the most energetic H;  M_{j_{b} j_{b}} (GeV); Number of events (Normalized to one)", 70, 70, 170)
        hist2 = ROOT.TH1F("hist2", "Invariant mass of the 2nd most energetic H;  M_{j_{b} j_{b}} (GeV); Number of events (Normalized to one)", 70, 70, 170)
...
for i, hist1 in enumerate(histograms_1):
    ...
    legend_PT = ROOT.TLegend(0.74
  , y1 - i * (y1 - y2) / n_entries, 0.92, y1 - (i + 1) * (y1 - y2) / n_entries)
...
################################################################## #NOVO PADRÃO
    ...
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(0, f"Mean: {mean_values_1[i]:.1f} GeV/c^{2} , Std Dev: {std_dev_values_1[i]:.1f} GeV/c^{2} ", "")
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(0, f"Min: {min_values1[i]:.1f} GeV/c^{2} , Max: {max_values1[i]:.1f} GeV/c^{2} ", "")
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(0, f"N.: {N_entries1[i]:.1f}, Mod. V: {modal_values_1[i]:.1f} GeV/c^{2} ", "")
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(0, "", "")
    legend_PT.Draw()
...
output_file_path_1 = os.path.join(output_dir, "mh1_fill_norm.png")
...
################################################################## #NOVO PADRÃO
    ...
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(0, f"Mean: {mean_values_2[i]:.1f} GeV/c^{2} , Std Dev: {std_dev_values_2[i]:.1f} GeV/c^{2} ", "")
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(0, f"Min: {min_values2[i]:.1f} GeV/c^{2} , Max: {max_values2[i]:.1f} GeV/c^{2} ", "")
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(0, f"N.: {N_entries2[i]:.1f}, Mod. V: {modal_values_2[i]:.1f} GeV/c^{2} ", "")
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(0, "", "")
    legend_PT.Draw()
...
canvas_2.Update()
# Salvando o histograma em um arquivo de imagem
output_file_path_2 = os.path.join(output_dir, "mh2_fill_norm.png")
