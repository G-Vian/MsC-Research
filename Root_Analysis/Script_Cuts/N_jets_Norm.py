...
output_dir = os.path.join(script_dir, "N_jetsN")
...
with open(output_file_path, "a") as output_file:
    ...
    for i, inputFile in enumerate(inputFiles):
        ...
        hist_jet_number = ROOT.TH1F("hist_jet_number", f"Number of Jets", 11, 5, 16)
        hist_b_number = ROOT.TH1F("hist_bjet_number", f"Number of b- Jets", 8, 3, 10)
        hist_q_number = ROOT.TH1F("hist_qjet_number", f"Number of q- Jets", 10, 0, 11)
    ...
    # A definição dos galhos deve ser feito uma única vez antes de iniciar o loop. Não é necessário (nem recomendado) chamar UseBranch a cada iteração do loop, pois isso pode causar sobrecarga.
        ...
        if hist_jet_number.Integral() > 0:
            hist_jet_number.Scale(1.0 / hist_jet_number.Integral())
        if hist_b_number.Integral() > 0:
            hist_b_number.Scale(1.0 / hist_b_number.Integral())
        if hist_q_number.Integral() > 0:
            hist_q_number.Scale(1.0 / hist_q_number.Integral())
...
min_nonzero_bin_content_j = float('inf')

for hist in [hist_jet_number]:
    for i in range(1, hist.GetNbinsX() + 1):
        bin_content = hist.GetBinContent(i)
        if bin_content > 0 and bin_content < min_nonzero_bin_content_j:
            min_nonzero_bin_content_j = bin_content

# Definir o limite inferior de y (use um fator de segurança se necessário)
y_min_j = min_nonzero_bin_content_j * 0.9 if min_nonzero_bin_content_j != float('inf') else 0.01# Encontrar o menor valor de bin não nulo para definir o limite inferior de y


# Encontrar o menor valor de bin não nulo para definir o limite inferior de y
min_nonzero_bin_content = float('inf')

for hist in [hist_b_number]:
    for i in range(1, hist.GetNbinsX() + 1):
        bin_content = hist.GetBinContent(i)
...
# Definir o limite inferior de y (use um fator de segurança se necessário)
y_min_b = min_nonzero_bin_content * 0.9 if min_nonzero_bin_content != float('inf') else 0.01# Encontrar o menor valor de bin não nulo para definir o limite inferior de y


# Encontrar o menor valor de bin não nulo para definir o limite inferior de y
min_nonzero_bin_content_q = float('inf')

for hist in [hist_q_number]:
    for i in range(1, hist.GetNbinsX() + 1):
        bin_content = hist.GetBinContent(i)
        if bin_content > 0 and bin_content < min_nonzero_bin_content_q:
            min_nonzero_bin_content_q = bin_content

# Definir o limite inferior de y (use um fator de segurança se necessário)
y_min_q = min_nonzero_bin_content_q * 0.9 if min_nonzero_bin_content_q != float('inf') else 0.01# Encontrar o menor valor de bin não nulo para definir o limite inferior de y
...
for i, hist_jet_number in enumerate(histograms_jets):
    ...
    hist_jet_number.SetLineWidth(7)
...
#        hist_jet_number.SetFillColor(colors[i])
    ...
    hist_jet_number.SetMinimum(y_min_j)
...
#################################################################
    ...
    legend_PT = ROOT.TLegend(0.81, y1 - i * (y1 - y2) / n_entries, 0.92, y1 - (i + 1) * (y1 - y2) / n_entries)
...
################################################################## #NOVO PADRÃO
    ...
    legend_PT.AddEntry(hist_jet_number, f"  {os.path.basename(inputFiles[i]).replace('100.root', '')}", "f100")
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(0, f"N.: {n_entries_jets[i]:.1f}", "")

    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(0, "", "")
    #legend_PT.AddEntry(0, f"Mean: {mean_values_1[i]:.1f} GeV, Std Dev: {std_dev_values_1[i]:.1f} GeV", "")
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(0, "", "")
    #legend_PT.AddEntry(0, f"Min: {min_values1[i]:.1f} GeV, Max: {max_values1[i]:.1f} GeV", "")
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(0, "", "")
...
output_file_path_HT = os.path.join(output_dir, "NjetsN.png")
...
for i, hist_bjet_number in enumerate(histograms_b):
    ...
    hist_bjet_number.SetLineWidth(7)
 ...
 #       hist_bjet_number.SetFillColor(colors[i])
    ...
    hist_bjet_number.SetMinimum(y_min_b)
...
################################################################## #NOVO PADRÃO
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(hist_bjet_number, f"  {os.path.basename(inputFiles[i]).replace('100.root', '')}", "f100")
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(0, f"N.: {n_entries_bjets[i]:.1f}", "")
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(0, "", "")
    #legend_PT.AddEntry(0, f"Mean: {mean_values_1[i]:.1f} GeV, Std Dev: {std_dev_values_1[i]:.1f} GeV", "")
    legend_PT.AddEntry(0, "", "")
...
# Salvando os histogramas em arquivos de imagem
output_file_path_HT2 = os.path.join(output_dir, "NbjetsN.png")
...
for i, hist_qjet_number in enumerate(histograms_q):
    ...
    hist_qjet_number.SetLineWidth(7)
...
#        hist_qjet_number.SetFillColor(colors[i])
    ...
    hist_qjet_number.SetMinimum(y_min_q)
...
################################################################## #NOVO PADRÃO
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(hist_qjet_number, f"  {os.path.basename(inputFiles[i]).replace('100.root', '')}", "f100")
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(0, f"N.: {n_entries_qjets[i]:.1f}", "")
    legend_PT.AddEntry(0, "", "")
    legend_PT.AddEntry(0, "", "")
    #legend_PT.AddEntry(0, f"Mean: {mean_values_1[i]:.1f} GeV, Std Dev: {std_dev_values_1[i]:.1f} GeV", "")
    legend_PT.AddEntry(0, "", "")
...
# Salvando os histogramas em arquivos de imagem
output_file_path_HT3 = os.path.join(output_dir, "NqjetsN.png")
